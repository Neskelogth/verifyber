...
def tract2standard_img_based(t_fn,
    ...
    if tx_type == 'SyNRA':
        ...
        mytx = ants.registration(fixed=fixed,
                                 ...
                                 flow_sigma=3,
                                 total_sigma=0,
                                 outprefix=f'{tmp_dir}/{sub}/ants_tract2standard',
                                 random_seed=SEED)
    elif tx_type == 'TRSAA':
        to_invert = [True]
        mytx = ants.registration(fixed=fixed,
    ...
    ants.image_write(mytx['warpedmovout'], f'{tmp_dir}/{sub}/struct_warped.nii.gz')

    print('correcting warp to mrtrix convention...')
    os.system(f'warpinit {fixed_fn} {tmp_dir}/{sub}/ID_warp[].nii.gz -force')

    for i in range(3):
        temp_warp = ants.image_read(f'{tmp_dir}/{sub}/ID_warp{i}.nii.gz')
        temp_warp = ants.apply_transforms(fixed=moving,
                                          moving=temp_warp,
                                          transformlist=mytx['invtransforms'],
                                          whichtoinvert=to_invert,
                                          defaultvalue=2147483647)
        ants.image_write(temp_warp, f'{tmp_dir}/{sub}/mrtrix_warp{i}.nii.gz')

    os.system(f'warpcorrect {tmp_dir}/{sub}/mrtrix_warp[].nii.gz ' +
              f'{tmp_dir}/{sub}/mrtrix_warp_cor.nii.gz ' +
              '-marker 2147483647 -tolerance 0.0001 -force')

    print('applying warp to tractogram...')
    os.system(
        f'tcktransform {t_fn} {tmp_dir}/{sub}/mrtrix_warp_cor.nii.gz {t_std_fn} ' +
        '-force -nthreads 0')
...
if __name__ == '__main__':
    ...
    assert len(cfg['trk']) == len(cfg['t1']) == len(cfg['subs'])

    exp = f'{script_dir}/checkpoints/{cfg["model"]}'
    cfg['exp_path'] = exp


    for trk, t1, sub in zip(cfg['trk'], cfg['t1'], cfg['subs']):

        trk = osp.abspath(trk)
        move_tract = cfg['warp'] != ''
        img_type = 'fa' if cfg['fa'] != '' else 't1'

        tck_fn = f'{tmp_dir}/{sub}/input/tract.tck'
        tck_mni_fn = f'{tck_fn[:-4]}_mni.tck'
        trk_mni_fn = trk
        trk_fn = f'{tmp_dir}/{sub}/input/tract_mni_resampled.trk'

        in_dir = f'{tmp_dir}/{sub}/input'
        ...
        if move_tract and not skip:
            ...
            if cfg['warp'] == 'slr':
                t0 = time()
                trk_mni_fn = tract2standard_sl_based(trk, mni_fn_dict['slr'],
                                                     f'{tck_mni_fn[:-4]}.trk')
        ...
        if not osp.exists(trk_fn):
            if not osp.exists(tck_mni_fn):
                ...
                os.system(f'''ln -sf {trk} {trk_fn}''')
            else:
                t0 = time()
                print('convert warped tck to trk...')
                tck2trk(tck_mni_fn, mni_fn_dict[img_type], out_fn=trk_fn)
                print(f'done in {time() - t0} sec')
        ...
        with torch.no_grad():
            ...
            for pred in preds:
                ...
                if cfg['return_trk']:
                    ...
                    tck = nib.streamlines.load(trk)
                    hdr = tck.header
                    streams = tck.streamlines
                    lengths = np.array([len(item) for item in streams])
                    streams = np.concatenate(streams, axis=0)

                    streamlines = np.split(streams, np.cumsum(lengths[:-1]))
                    streamlines = np.array(streamlines, dtype=object)[idxs_P]
                    out_t = nib.streamlines.Tractogram(streamlines, affine_to_rasmm=np.eye(4))
                    out_t_name = osbn(trk)[:-4] + '_filtered.trk'
                    out_t_fn = f'''{out_dir}/{out_t_name}'''

                    hdr['voxel_sizes'] = np.array([1., 1., 1.])
                    hdr['dimensions'] = np.array([182, 218, 182], dtype=np.int16)
